...
_train = pd.read_csv(trainFile)
_test = pd.read_csv(testFile)
_label = pd.read_csv(labelFile)

temp = _train

labels = _train.columns
trueInd = _train.notnull().all() & _test.notnull().all()   
trueInd['PassengerId','Ticket'] = False

temp = _train

title = '[a-zA-Z]*\.'

temp['Name'] = _train['Name'].str.findall(title).str[0].astype(str)

uniqueTitles = pd.unique(temp['Name'])

temp = pd.get_dummies(temp, columns=['Name', 'Sex'])

tempage = temp
tempind = tempage.notnull().all()
tempind['Age'] = True
